...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, depth=None):
        def stem(x):
            x = self.relu1(self.bn1(self.conv1(x) + self.conv1_alpha(depth)))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, depth=None, return_attn=False,pos_embed=None):
        x = self.conv1(x)  
        x = x.reshape(x.shape[0], x.shape[1], -1)  
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        depth_resized = F.adaptive_avg_pool2d(depth, (self.input_resolution // self.patch_size, self.input_resolution // self.patch_size))
        depth_resized = depth_resized.squeeze(1)
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        if return_attn:
            x, attn_last = self.transformer(x, return_attn=True,depth=depth_resized)
        else:
            x = self.transformer(x, return_attn=False,depth=depth_resized)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, depth):
        assert depth is not None
        return self.visual(image.type(self.dtype), depth.type(self.dtype))
    ...
    def forward(self, image, text, depth):
